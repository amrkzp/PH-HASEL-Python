...
class hasel_nn(nn.Module):
    def __init__(self, look_back):
        ...
        self.linear_relu_stack = nn.Sequential(
            nn.Linear(2*look_back, 64, dtype=float),
            nn.ReLU(),
            nn.BatchNorm1d(64, dtype=float),
    ...
    def forward(self, x):
        ...
        return self.linear_relu_stack(flattened)
...
def test_model(model, test_v, test_q, test_y, loss_fn, test_q_feedback):
    ...
    for i, v in enumerate(test_v):
        ...
        prediction = np.array([[model(torch.from_numpy(input_data)).item()]])
        prediction_feedback = np.array([[model(torch.from_numpy(input_data_feedback)).item()]])
        if (i == 0):
    ...
    y_hat_feedback_tensor = torch.tensor(y_hat_feedback)
...
def run_train_test(model, loss, optimizer,
    ...
    if (compare):
        if (not track_validation_loss or use_cached): #Run the test, either if we are using a cached model, or if we hadn't been tracking test losses when running the training model
            prediction, test_y, final_test_loss, prediction_feedback, final_test_loss_feedback = test_model(model, test_v, test_q, test_y, loss, test_q_feedback) #So that we don't end up calling it twice if we've been tracking test loss
        plot_dataset(test_y, prediction_feedback, 'actual', 'model')
        plt.show()
...
if __name__ == '__main__':
    ...
    # plot_dataset(master_V, master_q)

    #Divide into train and test set
    train_V, train_q, test_V, test_q_orig = split_dataset(master_V, master_q, train_ratio)
    del master_V
    # plot_dataset(train_V, train_q)
    # plot_dataset(test_V, test_q)
    # plt.show()
    ...
    model_name = 'Models/DNNs/Sine/Downsampled/BEST_DNN_Sine_100_Downsampled/Model_2_Sine_100_DS' #Model to load from cache
